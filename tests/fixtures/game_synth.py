...
def build_rom(with_game_over: bool = False) -> bytes:
    ...
    reset_code = [
        ...
        0x01,
        0x85,
        ZP_LEVEL,  # LDA #1 ; STA level
        0xAD,
        0x02,
        0x20,  # LDA $2002 (clear vblank latch)
    ]
    reset_off + len(reset_code)
    reset_code += [
        0xAD,
        0x02,
        0x20,  # LDA $2002
        ...
        0x8D,
        0x00,
        0x20,  # LDA #$80 ; STA PPUCTRL  (enable NMI)
    ]
    ...
    nmi_code = [
        ...
        0x40,  # LDA #0 ; STA $4016
        0x85,
        ZP_CONTROLLER1,  # STA controller1 (clear before ROL)
        0xA2,
        0x08,  # LDX #8
    ]
    len(nmi_code)
    nmi_code += [
        ...
        0x4A,  # LSR A
        0x26,
        ZP_CONTROLLER1,  # ROL controller1
        0xCA,  # DEX
        0xD0,
        (256 - 9) & 0xFF,  # BNE -9 → top of read loop (loop body = 9 bytes)
    ]
    if with_game_over:
        nmi_code += [
            0xA5,
            ZP_CONTROLLER1,
            0x29,
            0x80,  # LDA controller1 ; AND #$80
            0xF0,
            0x16,  # BEQ +22 (skip A handling)
            0xC6,
            ZP_LIVES,  # DEC lives
            0x10,
            0x12,  # BPL +18 (lives ≥0, skip reset)
            0xA9,
            0x03,
            0x85,
            ZP_LIVES,  # LDA #3 ; STA lives
            ...
            0x00,
            0x85,
            ZP_SCORE_LO,  # LDA #0 ; STA score_lo
            0x85,
            ZP_SCORE_HI,  # STA score_hi
            0xA9,
            0x01,
            0x85,
            ZP_LEVEL,  # LDA #1 ; STA level
            ...
            0x00,
            0x85,
            ZP_FRAME_COUNTER,  # LDA #0 ; STA frame_counter
        ]
    else:
        nmi_code += [
            0xA5,
    ...
    nmi_code += [
        ...
        ZP_CONTROLLER1,
        0x29,
        0x40,  # LDA controller1 ; AND #$40
        0xF0,
        0x06,  # BEQ +6
        0xE6,
        ZP_SCORE_LO,  # INC score_lo
        0xD0,
        0x02,  # BNE +2
        0xE6,
        ZP_SCORE_HI,  # INC score_hi
    ]
    nmi_code += [
        0xA5,
        ZP_CONTROLLER1,
        0x29,
        0x10,  # LDA controller1 ; AND #$10 (Start)
        0xF0,
        0x02,  # BEQ +2
        0xE6,
        ZP_LEVEL,  # INC level
