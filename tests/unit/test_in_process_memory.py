...
def test_mapper19_namco163_switches_prg_and_1k_chr_windows():
    banks = [bytes([bank_id] * 0x2000) for bank_id in range(8)]
    chr_data = b"".join(bytes([bank_id] * 0x0400) for bank_id in range(32))
    m = Namco163Memory(b"".join(banks), chr_data=chr_data)
    assert m[0x8000] == 0
    assert m[0xA000] == 1
    assert m[0xC000] == 2
    assert m[0xE000] == 7
...
def test_mapper24_vrc6_switches_prg_and_1k_chr_windows():
    banks = [bytes([bank_id] * 0x2000) for bank_id in range(8)]
    chr_data = b"".join(bytes([bank_id] * 0x0400) for bank_id in range(32))
    m = VRC6Memory(b"".join(banks), chr_data=chr_data, mapper=24)
    assert m[0x8000] == 0
    assert m[0xA000] == 1
    assert m[0xC000] == 0
    assert m[0xE000] == 7
...
def test_mapper26_vrc6_swaps_a0_a1_register_decode():
    banks = [bytes([bank_id] * 0x2000) for bank_id in range(8)]
    chr_data = b"".join(bytes([bank_id] * 0x0400) for bank_id in range(32))
    m = VRC6Memory(b"".join(banks), chr_data=chr_data, mapper=26)
...
def test_mapper75_vrc1_switches_prg_and_4k_chr_windows():
    banks = [bytes([bank_id] * 0x2000) for bank_id in range(8)]
    chr_data = b"".join(bytes([bank_id] * 0x1000) for bank_id in range(32))
    m = VRC1Memory(b"".join(banks), chr_data=chr_data)
    assert m[0x8000] == 0
    assert m[0xA000] == 1
    assert m[0xC000] == 2
    assert m[0xE000] == 7
    ...
    m[0xF000] = 0x02
    snap = m.ppu_snapshot()
    assert snap.pattern_table[0x1000] == 18
    assert snap.pattern_table[0x1FFF] == 18


def test_mapper32_irem_g101_switches_prg_mode_and_1k_chr_windows():
    banks = [bytes([bank_id] * 0x2000) for bank_id in range(8)]
    chr_data = b"".join(bytes([bank_id] * 0x0400) for bank_id in range(32))
    m = IremG101Memory(b"".join(banks), chr_data=chr_data)
    assert m[0x8000] == 0
    assert m[0xA000] == 1
    assert m[0xC000] == 6
    assert m[0xE000] == 7
