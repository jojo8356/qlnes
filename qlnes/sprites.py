    ...
    for raw_entry in text.replace(",", " ").split():
        ...
        for button in buttons_text.split("+"):
            ...
            try:
                ...
                raise ValueError(
                    f"unknown runtime input button {button!r}; valid: {valid}"
                ) from exc
        if mask == 0:
...
def sprite_palette_metadata(
    ...
    ppu_values = [
        palette_ram[0x10 + palette_id * 4 + color_index] & 0x3F for color_index in range(4)
    ]
    return {
    ...
    *,
    ...
    for rows, sprite in reversed(list(zip(decoded_sprites, exports, strict=True))):
        ...
        if left >= right or top >= bottom:
            continue
        crop = sprite_img.crop(
            (left - sprite.x, top - sprite.y, right - sprite.x, bottom - sprite.y)
        )
...
def export_in_process_runtime_sprites(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="qlnes-sprite-snapshot-") as td:
        ...
        if (
            not rom.header
            or rom.header.chr_size == 0
            or rom.mapper
            in (1, 4, 5, 9, 10, 16, 18, 19, 21, 22, 23, 24, 25, 26, 32, 33, 34, 64, 69, 75, 85, 206)
        ):
                ...
                "samples": [
                    {
                        ...
                        "spritesheet": (str(sample.spritesheet) if sample.spritesheet else None),
        ...
        try:
            if runtime_sample_frames is not None:
                ...
                if sampled.manifest_json is not None:
                    sampled_data = json.loads(sampled.manifest_json.read_text(encoding="utf-8"))
        ...
        json.dumps(
            {
                ...
                "all_unique_trimmed_dir": (
                    str(out_dir / "all-unique-trimmed") if batch.all_unique_trimmed else None
                ),
